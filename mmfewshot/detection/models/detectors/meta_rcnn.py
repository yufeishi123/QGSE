...
from ..builder import build_BGSM
...
class MetaRCNN(QuerySupportDetector):
    ...
    def __init__(self,
                 ...
                 information_QGM: Optional[ConfigDict] = None,
                 information_BGSM: Optional[ConfigDict] = None,
        ...
        if information_BGSM is not None:
            self.BGSM = build_BGSM(information_BGSM)
            self.depthwise = nn.Sequential(
                nn.Conv2d(in_channels=1024, out_channels=1024, kernel_size=3, stride=1, padding=1, groups=1024),
                nn.BatchNorm2d(1024),
                nn.ReLU()
            )
        ...
        if information_QGM is not None:
            self.with_information_coupling = True
            self.QGM = build_QGM(information_QGM)
    ...
    def forward_train(self,
        ...
        query_feats = self.depthwise(query_feats[0])
        query_feats = self.pointwise(query_feats)
        query_feats = self.BGSM(query_feats)
        ...
        if not self.with_information_coupling:
            ...
            for img_id in range(batch_size):
                ...
                support_feats_ = self.QGM(support_feats_origin[0], query_feats_)
    ...
    def simple_test(self,
        ...
        for class_id, sup_feats in self.inference_support_feats_dict.items():
            if sup_feats.size(0) > 30:
                ...
                feats = sup_feats
            support_feats = self.QGM(feats, query_feats[0].repeat(feats.size(0), 1, 1, 1))
