...
from ..builder import build_VSEM
...
class MetaRCNNRoIHead(StandardRoIHead):
    ...
    def __init__(self,
                 ...
                 information_VSEM: Optional[ConfigDict] = None,
        ...
        if information_VSEM is not None:
            self.VSEM = build_VSEM(information_VSEM)
    ...
    def _bbox_forward_train(self, query_feats: List[Tensor],
        ...
        bbox_results = None
        support_gt_labels_slice = support_gt_labels[::2]
        for img_id in range(batch_size):
            ...
            support_feat = support_feat_all[img_id].view(15, 2, 2048).mean(dim=1)
            support_feat = self.VSEM(support_feat)
            for i in range(support_feat.size(0)):
                ...
                if support_gt_labels_slice[i] == random_query_label:
    ...
    def simple_test_bboxes(
        ...
        for class_id in support_feats_dict.keys():
            ...
            support_feat = self.VSEM(support_feat).mean(dim=0).unsqueeze(0)
